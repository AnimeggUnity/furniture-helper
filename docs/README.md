...
#### 📊 資料面板 (Data Panel)
- **功能**: 在頁面右側生成一個側邊面板，直接展示當前列表上**所有商品**的完整競標紀錄（包含最高出價者與價格）。
- **解決痛點**: 無需手動一個個點擊「查看競標紀錄」的彈出視窗，所有資訊一目了然，極大化提升了分析效率。
- **實現方式**: 點擊按鈕後，腳本會自動收集所有商品的 `UUID`，並行地向後端 API (`/api/Product/GetBidLog`) 發送請求，獲取資料後整理並渲染至面板上。

#### 📈 年度統計 (Annual Statistics)
- **功能**: 對當前頁面表格中的資料進行快速統計與分析，並提供將結果匯出為 `.csv` 檔案的選項。
- **解決痛點**: 自動計算總覽數據，取代手動複製貼上到 Excel 進行分析的繁瑣流程。
...
#### 📥 匯入 JSON (Import JSON)
- **功能**: 允許使用者選擇一個本地的 `JSON` 檔案，腳本會自動將檔案中的資料填入「新增/編輯」表單對應的欄位中。
- **解決痛點**: 徹底解決手動填寫大量欄位的耗時與高錯誤率。腳本特別針對 Element UI 的複雜下拉選單（如類別、行政區）和日期選擇器進行了優化，透過**串行執行**與**動態輪詢**確保了在複雜介面下的穩定性與可靠性。

#### 🚀 直接匯入 (Direct Import)
- **功能**: 這是「匯入JSON」的進階版。使用者選擇一個包含 Base64 圖片資料的 `JSON` 檔案後，腳本會**完全繞過前端表單**，直接呼叫後端 API (`/api/Product/AddProduct`) 來提交資料。
- **解決痛點**: 提供最極致的上架效率。腳本會自動處理圖片上傳、更新拍賣日期，並在成功後自動刷新頁面，實現「一鍵上架」。

#### ☁️ 遠端匯入 (Remote Import)
- **功能**: 從指定的遠端伺服器 (`https://580.blias.com/daobo/files.php`) 獲取可用的 `JSON` 檔案清單，並在側邊面板中展示。使用者可以搜尋並選擇其中任一筆資料，觸發「直接匯入」流程。
- **解決痛點**: 實現了資料的雲端管理與團隊協作，無需透過通訊軟體手動傳輸 `JSON` 檔案。

---

### 3. 輔助功能

#### 🖼️ 圖片預覽與上傳 (Image Preview & Upload)
- **功能**: 在執行「直接匯入」或「遠端匯入」之前，會彈出一個預覽視窗，顯示即將上傳的所有圖片。圖片上傳功能會自動將 `JSON` 中的 Base64 字串轉換為可上傳的檔案格式。
- **解決痛點**: 在提交前提供一次視覺確認的機會，確保上傳的圖片正確無誤。

#### 🖨️ 得標者資訊增強 (Winner Info Enhancement)
- **功能**: 在「列印」頁面，將原本顯示得標者數字 ID 的欄位，自動替換為更有意義的「暱稱 (帳號)」格式。
- **解決痛痛點**: 提升了列印資料的可讀性。
