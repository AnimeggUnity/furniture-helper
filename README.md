...
- **`components/`**:
    - **`dataPanel.js`**: `buildPanel` 函數，建立側邊資料面板，包含搜尋、三模式選擇按鈕（全選/全不選/只選無競標）、批次操作等功能。🆕
    - **`statsModal.js`**: `showHierarchicalModal` 函數，建立階層式統計資料的互動式樹狀圖。
    ...
    - **`settingsPanel.js`**: `showSettingsPanel` 函數，提供 Webhook 設定的 UI。
...
2. **預設行為**: 開啟資料面板時，自動勾選所有無競標項目
3. **即時計數**: 顯示「已選 X 項」，獨立一行不影響排版
4. **競標狀態顯示**:
