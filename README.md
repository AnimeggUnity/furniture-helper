...
├── content.js             # 主要業務邏輯 (2,084行)
├── constants.js           # 應用常數與樣式系統 (486行)
├── utils.js               # 純函數與通用工具 (248行)
├── api.js                 # API 相關功能 (270行)
├── inject.js              # Vue 資料提取腳本 (342行)
└── docs/
    └── README.md          # 完整說明文件
```
...
#### 模組化重構
- **🆕 constants.js** - 統一應用常數、UI 元件樣式系統、競標狀態機、錯誤處理系統
- **🆕 utils.js** - 純函數工具庫、Webhook 管理、通知系統、Base64 處理
- **🆕 api.js** - 集中 API 調用、圖片上傳、批次操作功能
- **content.js** - 精簪主要業務邏輯，從 3000+ 行優化至 2084 行
- **inject.js** - 強化 Vue 資料存取，新增競標資料查詢與進度回報
...
#### 5. 資料管理模組 🆕
- **智能選擇機制** - 自動排除有競標者的項目，避免誤操作
- **批次刪除功能** - 支援多項目批次刪除，含確認對話框與安全機制
- **競標狀態檢查** - 實時檢查項目競標狀態，智能預設選擇
...
### v5.0.0 (Current - September 2024) 🆕
**模組化架構重構與進階功能**：
- 🏗️ **程式碼模組化**：拆分 content.js 為 4 個專業模組 (constants.js, utils.js, api.js)
- 🎯 **自訂競標天數**：遠端匯入支援 1-90 天自訂競標期間，預設 14 天
- 📊 **智能列印表格**：整合 GetBidLog API 顯示完整競標資訊，含實時進度條
- 🚫 **進度條中斷功能**：可隨時取消耗時的列印作業，避免伺服器過載
- 🎨 **UI/UX 優化**：競標資訊一行顯示、移除圖示、淡紅底色標示
- 📡 **批次請求優化**：3個/批次, 200ms間隔，防止類似 DDOS 攻擊
- 🔗 **預設 Webhook 更新**：https://580.blias.com/daobo/files.php?format=json
